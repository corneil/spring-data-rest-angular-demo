    ...
    angular.module('springDataRestDemo').service('MemberService', ['$q', '$resource', '$log', 'UserService', 'GroupService',
        function ($q, $resource, $log, UserService, GroupService) {
            ...
            function loadGroup(groupMap, member) {
                ...
                GroupService.loadGroup(member._links._group.href).then(function (group) {
                    var groupInfo = {};
            ...
            function loadAndAddUser(groupMember) {
                var deferred = $q.defer();
                UserService.loadUser(groupMember._member._links._user.href).then(function (user) {
            ...
            function loadGroupMembers(group) {
                ...
                if(group.members == undefined || group.members == null) {
                    var MembersForGroup = $resource(group._group._links.self.href + '/members');
                    MembersForGroup.get().$promise.then(function (groupMembers) {
                        group.members = [];
                        if(groupMembers._embedded) {
                            var members = groupMembers._embedded.members;
